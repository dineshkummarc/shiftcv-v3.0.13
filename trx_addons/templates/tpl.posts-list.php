...
$trx_addons_output .= '<div class="post_content">'
			. ($trx_addons_show_categories 
					? '<div class="post_categories">'.trx_addons_get_post_categories().trim($trx_addons_post_counters_output).'</div>' 
					: '')
			...
			. ($trx_addons_show_rating && function_exists('trx_addons_sc_reviews')
					? trx_addons_sc_reviews(array(	'post_id' => $trx_addons_post_id,
													'rating_max_level' => 5,
													'rating_style' => 'icons',
													'rating_text_template' => '#',	// Don't display text
													'allow_voting' => 0
													))
					: ''
					)
