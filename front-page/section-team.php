<div class="front_page_section front_page_section_team<?php
	$shift_cv_scheme = shift_cv_get_theme_option( 'front_page_team_scheme' );
	if ( ! shift_cv_is_inherit( $shift_cv_scheme ) ) {
		echo ' scheme_' . esc_attr( $shift_cv_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( shift_cv_get_theme_option( 'front_page_team_paddings' ) );
?>"
		<?php
		$shift_cv_css      = '';
		$shift_cv_bg_image = shift_cv_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$shift_cv_anchor_icon = shift_cv_get_theme_option( 'front_page_team_anchor_icon' );
	$shift_cv_anchor_text = shift_cv_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( shift_cv_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$shift_cv_css      = '';
			$shift_cv_bg_mask  = shift_cv_get_theme_option( 'front_page_team_bg_mask' );
			$shift_cv_bg_color_type = shift_cv_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $shift_cv_bg_color_type ) {
				$shift_cv_bg_color = shift_cv_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$shift_cv_caption = shift_cv_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $shift_cv_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $shift_cv_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $shift_cv_caption, 'shift_cv_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$shift_cv_description = shift_cv_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $shift_cv_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $shift_cv_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $shift_cv_description ), 'shift_cv_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
				...
				if ( ! shift_cv_exists_trx_addons() ) {
					...
					shift_cv_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
				}
			}
			?>
			</div>
