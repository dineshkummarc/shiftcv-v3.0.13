...
// Theme storage
//-------------------------------------------------------------------------
shift_cv_storage_set('theme_demo_url', '//financial-adviser.shift-cv.themerex.net');
...
if ( ! function_exists( 'shift_cv_skin_importer_set_options' ) ) {
	...
	function shift_cv_skin_importer_set_options($options = array()) {
		if (is_array($options)) {
			// Corporate demo
            $options['demo_type'] = 'financial-adviser';
			...
			$options['files']['financial-adviser']['domain_demo'] = esc_url( 'https://financial-adviser.shift-cv.themerex.net' );   // Demo-site domain
		}
		return $options;
	}
}
