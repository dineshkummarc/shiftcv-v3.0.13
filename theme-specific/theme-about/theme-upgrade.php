...
if ( ! function_exists( 'shift_cv_pro_get_pro_version_callback' ) ) {
	...
	function shift_cv_pro_get_pro_version_callback() {
		if ( ! wp_verify_nonce( shift_cv_get_value_gp( 'nonce' ), admin_url( 'admin-ajax.php' ) ) || ! current_user_can( 'manage_options' ) ) {
			wp_die();
		}
		...
		if ( ! empty( $key ) ) {
			...
			$upgrade_url = sprintf(
				'https://upgrade.themerex.net/upgrade.php?key=%1$s&src=%2$s&theme_slug=%3$s&theme_name=%4$s',
				urlencode( $key ),
