						...
						shift_cv_storage_set_array2(
							'options', $k, 'desc', $v['desc']
												...
												. sprintf(
													...
													esc_html__( 'Open selected layout in a new tab to edit', 'shift-cv' )
												)
						);
					}
				}
...
if ( ! function_exists( 'shift_cv_trx_addons_default_layouts' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_default_layouts',	'shift_cv_trx_addons_default_layouts');
	function shift_cv_trx_addons_default_layouts( $default_layouts = array() ) {
		if ( shift_cv_storage_isset( 'trx_addons_default_layouts' ) ) {
			...
			include_once shift_cv_get_file_dir( 'theme-specific/trx_addons-layouts.php' );
			if ( ! isset( $layouts ) || ! is_array( $layouts ) ) {
				$layouts = array();
			} else if ( function_exists( 'trx_addons_url_replace' ) ) {
				// Replace demo-site urls with current site url
				$layouts = trx_addons_url_replace( shift_cv_storage_get( 'theme_demo_url' ), get_home_url(), $layouts );
			}
