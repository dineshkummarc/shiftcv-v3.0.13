	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="shift_cv_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.shift_cv_post_editor' );
					if ( link.length > 0 ) {
						shift_cv_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.shift_cv_post_editor', function(e) {
					shift_cv_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('shift_cv_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
